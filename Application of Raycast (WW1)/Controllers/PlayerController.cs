...
public class PlayerController : MonoBehaviour
{
    ...
    void PerformRaycast()
    {
        if (coinManager.currentLevelAt == 1) // Applies the laser effect only on the first level
        {
            ...
            if (coinManager.coinCount >= 1 && instructionTime == 0)
            {
                StartCoroutine(PerformInstruction());
                instructionTime++;
            }

            if (coinManager.coinCount >= 1 && Input.GetKeyDown(KeyCode.K)) // Checks if the player has at least 1 coin and "K" key on the keyboard is pressed
            {
